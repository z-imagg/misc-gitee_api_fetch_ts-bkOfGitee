...
class ResponseWrapT {
  ...
  reqId:DP.Protocol.Network.RequestId;
  response:DP.Protocol.Network.Response;


  // 构造函数
  constructor( reqId:DP.Protocol.Network.RequestId, response:DP.Protocol.Network.Response ) {
    this.reqId = reqId
...
const responseLs:Map<DP.Protocol.Network.RequestId,ResponseWrapT[]>=new Map();
...
function pushResponse( reqId:DP.Protocol.Network.RequestId, response:DP.Protocol.Network.Response){
  let ls:ResponseWrapT[]=responseLs.get(reqId)
  if(ls==null){
    responseLs.set(reqId,[]);
    ls=responseLs.get(reqId)
  }
  ls.push(new ResponseWrapT(reqId,response ))
}
